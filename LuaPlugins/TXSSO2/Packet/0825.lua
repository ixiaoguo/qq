    ...
    if lvl >= alvlC then
      ...
      if func then
        ...
        if func then
          func = func[ tag ];
          if func then
            if not pcall( func, data, pkg, root, ttt, off + 2 + 2, len ) then
              TreeAddEx( fieldsex, ttt, data( off + 2 + 2, len ), ">unsolved", len );
            end
          else
...
dissectors[0x3649][0x0825].recv = function( buf, pkg, root, t )
  ...
  if ds == nil or #ds == 0 then
    t:add( buf( 0xE, rest ), string.format(
  ...
  local tt = t:add( buf( 0xE, rest ), info );
  
  local off = 0;
  off = TreeAddEx( fieldsex, tt, data, off, ">cResult" );
